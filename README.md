...
이 프로그램을 처음 만들 때 매일 의무적으로 해야하는 3시간 분량의 편집이 그저 지루했습니다. 그래서 '이거 어쩌면 자동화 할 수 있지 않을까?' 라는 생각이 들었고, 실천했습니다. 유튜브에서 [나도코딩의 파이썬](https://youtu.be/kWiCuklohdY?si=2t6BNg9ZRLJ98gAe) 강의를 보았고, 자동편집 프로그램을 만들며 정말 재밌었습니다.
...
저는 앞에 보이는 나무들만을 보며 걸어갔습니다. 숲은 엄청 큰 데 말이죠. 지금에야 이 프로그램에 있어선 숲을 빠져나와 왔던 길이 잘 보입니다. 비록 AutoEditor라는 숲을 빠져나왔지만, 다른 프로젝트를 하면 다시 앞에 보이는 나무를 보며 숲을 빠져나와야겠죠. 저는 앞으로 여러 숲을 빠져나오고 길을 훤히 내다볼 수 있는 사람이 되면 좋겠는 바람입니다.
